	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function run( $options ) {
				...
				$workite_dir = $this->unpack_package( $download, $delete_package );
				if ( is_wp_error( $workite_dir ) ) {
					$this->skin->error( $workite_dir );
					$this->skin->after();
					return $workite_dir;
				}
			...
			public function install_strings() {
				...
				$this->strings['downloadite_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', TGM_Plugin_Activation::$instance->domain );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', TGM_Plugin_Activation::$instance->domain );
				$this->strings['installite_package']  = __( 'Installing the plugin&#8230;', TGM_Plugin_Activation::$instance->domain );
